...
static void callback(const sensor_msgs::PointCloud2ConstPtr& input_lidar) //0312 datacollection pretest
{
...
// Send TF "/velodyne" to "/map"
    ...
    if (_use_local_transform == true)
    {
      br_velodyne.sendTransform(tf::StampedTransform(local_transform * transform, current_scan_time, "/map", "/velodyne"));
    }
    else
    {
      br_velodyne.sendTransform(tf::StampedTransform(transform, current_scan_time, "/map", "/velodyne"));
    }
...
*/
  ...
  */
  ...
  ros::Subscriber points_sub = nh.subscribe("/velodyne_points", 1,callback);
