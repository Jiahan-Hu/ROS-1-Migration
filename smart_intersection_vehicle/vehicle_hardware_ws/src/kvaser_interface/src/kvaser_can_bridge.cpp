...
std::string can_tx_str;
std::string can_rx_str;
...
int main(int argc, char** argv)
{
  ...
  priv.param<std::string>("can_tx_topic", can_tx_str, "/can_tx");
  priv.param<std::string>("can_rx_topic", can_rx_str, "/can_rx");
  
  can_tx_pub = n.advertise<can_msgs::Frame>(can_tx_str, 500);
  ros::Subscriber can_rx_sub = n.subscribe(can_rx_str, 500, can_rx_callback);
