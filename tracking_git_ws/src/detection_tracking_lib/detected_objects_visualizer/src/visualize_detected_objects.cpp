...
VisualizeDetectedObjects::VisualizeDetectedObjects() : arrow_height_(0.5), label_height_(1.0)
{
  ...
  std::string markers_out_topic_label=markers_out_topic+"/label";
  std::string markers_out_topic_arrow=markers_out_topic+"/arrow";
  std::string markers_out_topic_boxes=markers_out_topic+"/boxes";
  std::string markers_out_topic_models=markers_out_topic+"/models";
  std::string markers_out_topic_hulls=markers_out_topic+"/hulls";
  std::string markers_out_topic_centroids=markers_out_topic+"/centroids";


  publisher_markers_ToLabels= node_handle_.advertise<visualization_msgs::MarkerArray>(
    markers_out_topic_label, 1);
  publisher_markers_ToArrows= node_handle_.advertise<visualization_msgs::MarkerArray>(
    markers_out_topic_arrow, 1);
  publisher_markers_ToBoxes= node_handle_.advertise<visualization_msgs::MarkerArray>(
    markers_out_topic_boxes, 1);
  publisher_markers_ToModels= node_handle_.advertise<visualization_msgs::MarkerArray>(
    markers_out_topic_models, 1);
  publisher_markers_ToHulls= node_handle_.advertise<visualization_msgs::MarkerArray>(
    markers_out_topic_hulls, 1);
  publisher_markers_ToCentroids= node_handle_.advertise<visualization_msgs::MarkerArray>(
    markers_out_topic_centroids, 1);
...
VisualizeDetectedObjects::ObjectsToCentroids(const autoware_msgs::DetectedObjectArray &in_objects)
{
  ...
  clear_marker.header = in_objects.header;
  clear_marker.ns = ros_namespace_ + "/centroid_markers";
  clear_marker.id = 0;
  clear_marker.action = clear_marker.DELETEALL;
  clear_marker.lifetime = ros::Duration();
  empty_markers.markers.push_back(clear_marker);
  publisher_markers_ToCentroids.publish(empty_markers);
  ...
  for (auto const &object: in_objects.objects)
  {
    if (IsObjectValid(object))
    {
      ...
      // centroid_marker.lifetime = ros::Duration();
  ...
  {
    publisher_markers_ToCentroids.publish(centroid_markers);
  }  
...
VisualizeDetectedObjects::ObjectsToBoxes(const autoware_msgs::DetectedObjectArray &in_objects)
{
  ...
  visualization_msgs::Marker clear_marker;
  clear_marker.header = in_objects.header;
  clear_marker.ns = ros_namespace_ + "/box_markers";
  clear_marker.id = 0;
  clear_marker.action = clear_marker.DELETEALL;
  clear_marker.lifetime = ros::Duration();
  empty_markers.markers.push_back(clear_marker);
  publisher_markers_ToBoxes.publish(empty_markers);
  ...
  if (!object_boxes.markers.empty())
  {
    publisher_markers_ToBoxes.publish(object_boxes);
  }
  return object_boxes;
}//ObjectsToBoxes
...
VisualizeDetectedObjects::ObjectsToModels(const autoware_msgs::DetectedObjectArray &in_objects)
{
  ...
  visualization_msgs::Marker clear_marker;
  clear_marker.header = in_objects.header;
  clear_marker.ns = ros_namespace_ + "/model_markers";
  clear_marker.id = 0;
  clear_marker.action = clear_marker.DELETEALL;
  clear_marker.lifetime = ros::Duration();
  empty_markers.markers.push_back(clear_marker);
  publisher_markers_ToModels.publish(empty_markers);
  ...
  if (!object_models.markers.empty())
  {
    publisher_markers_ToModels.publish(object_models);
  }
...
VisualizeDetectedObjects::ObjectsToHulls(const autoware_msgs::DetectedObjectArray &in_objects)
{
  ...
  visualization_msgs::Marker clear_marker;
  clear_marker.header = in_objects.header;
  clear_marker.ns = ros_namespace_ + "/hull_markers";
  clear_marker.id = 0;
  clear_marker.action = clear_marker.DELETEALL;
  clear_marker.lifetime = ros::Duration();
  empty_markers.markers.push_back(clear_marker);
  publisher_markers_ToHulls.publish(empty_markers);
  ...
  if (!polygon_hulls.markers.empty())
  {
    publisher_markers_ToHulls.publish(polygon_hulls);
  }
...
VisualizeDetectedObjects::ObjectsToArrows(const autoware_msgs::DetectedObjectArray &in_objects)
{
  ...
  visualization_msgs::Marker clear_marker;
  clear_marker.header = in_objects.header;
  clear_marker.ns = ros_namespace_ + "/arrow_markers";
  clear_marker.id = 0;
  clear_marker.action = clear_marker.DELETEALL;
  clear_marker.lifetime = ros::Duration();
  empty_markers.markers.push_back(clear_marker);
  publisher_markers_ToArrows.publish(empty_markers);
  ...
  {
    publisher_markers_ToArrows.publish(arrow_markers);
  }   
...
VisualizeDetectedObjects::ObjectsToLabels(const autoware_msgs::DetectedObjectArray &in_objects)
{
  ...
  visualization_msgs::Marker clear_marker;
  clear_marker.header = in_objects.header;
  clear_marker.ns = ros_namespace_ + "/label_markers";
  clear_marker.id = 0;
  clear_marker.action = clear_marker.DELETEALL;
  clear_marker.lifetime = ros::Duration();
  empty_markers.markers.push_back(clear_marker);
  publisher_markers_ToLabels.publish(empty_markers);
  ...
  for (auto const &object: in_objects.objects)
  {
    if (IsObjectValid(object))
    {
      ...
      // label_marker.lifetime = ros::Duration();
  ...
  if (!label_markers.markers.empty())
  {
    publisher_markers_ToLabels.publish(label_markers);
  }
