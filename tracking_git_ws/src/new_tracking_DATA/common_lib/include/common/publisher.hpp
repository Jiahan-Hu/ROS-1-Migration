...
#include "common/msgs/visualization_header_msgs/MarkerArrayHeader.h"
...
static bool IsObjectValid(const autoware_msgs::DetectedObject &in_object)
{
  if (!in_object.valid ||
...
static void publishObjectsMarkers(
    ...
    if (objects_array.size() <= 0) {
        ...
        ROS_INFO("publishObjectsMarkers Publishing %lu objects markers.", objects_array.size());
    }
...
// zzl modified
static void publishObjectsMarkersBox(
    const ros::Publisher &publisher,
    const std_msgs::Header &header,
    const std_msgs::ColorRGBA &color,
    const std::vector<ObjectPtr> &objects_array,
    ...
    if (objects_array.size() <= 0) {
        ...
        ROS_INFO("MarkersBox Publishing %lu objects markers.", objects_array.size());
    }
    ...
    for (size_t obj = 0u; obj < objects_array.size(); ++obj) {
        ...
        box.pose.position.x = center(0);
        box.pose.position.y = center(1);
        box.pose.position.z = center(2) + height / 2;
        box.pose.orientation = tf::createQuaternionMsgFromYaw(yaw);
        box.scale.x = length;
        box.scale.y = width;
        box.scale.z = height;
...
// zzl modified
static void publishObjectsMarkersBoxScore(
    const ros::Publisher &publisher,
    const std_msgs::Header &header,
    const std_msgs::ColorRGBA &color,
    const std::vector<ObjectPtr> &objects_array,
    ...
    if (objects_array.size() <= 0) {
        ...
        ROS_INFO("------------------------");
        ROS_INFO("MarkersBoxScore Publishing %lu objects markers.", objects_array.size());
        ROS_INFO("MarkersBoxScore in_objects size: %lu", in_objects.objects.size());
    }

    visualization_msgs::MarkerArray object_markers;
    for (size_t obj = 0u; obj < objects_array.size(); ++obj) {

        // get the object from in_objects.objects using index obj 
        
        // for (size_t i = 0; i < in_objects.objects.size(); ++i) {
        //     if (in_objects.objects[i].id == objects_array[obj]->id) {
        //         in_object = in_objects.objects[i];
        //         break;
        //     }
        // }
        // in_objects id are all 0, so use the index instead
        // the index is not correct, so use position matching instead 
        const double object_max_linear_size_ = 50.;
        

        // in_object = in_objects.objects[obj];
        // if (IsObjectValid(in_object) &&
        //     (in_object.pose_reliable || in_object.label != "unknown") &&
        //     (in_object.dimensions.x + in_object.dimensions.y + in_object.dimensions.z) < object_max_linear_size_)
        // {
        //     // get the score from in_object.score
        //     score_str = std::to_string(in_object.score);
        // }else{
        //     score_str = "0.0";
        // }    
        ...
        visualization_msgs::Marker box, dir_arrow;
        box.header = dir_arrow.header = header;
        box.ns = dir_arrow.ns = "objects/" + header.frame_id;
        // This is not the tracking id.
        box.id = obj;
        // change the id to the id of in_objects.objects
        // box.id = objects_array[obj]->id;
        box.pose.position.x = center(0);
        box.pose.position.y = center(1);
        box.pose.position.z = center(2) + height / 2;
        box.pose.orientation = tf::createQuaternionMsgFromYaw(yaw);
        ...
        box.color = color;
        box.color.a = 0.5;
        // put the score_str on the box.text
        autoware_msgs::DetectedObject in_object;
        for (size_t i = 0; i < in_objects.objects.size(); ++i) {
            double x_diff = abs(in_objects.objects[i].pose.position.x - box.pose.position.x);
            double y_diff = abs(in_objects.objects[i].pose.position.y - box.pose.position.y);
            // double z_diff = abs(in_objects.objects[i].pose.position.z - box.pose.position.z);
            if (x_diff < 1 && y_diff < 1) {
                in_object = in_objects.objects[i];
                break;
            }
        }        
        std::string score_str="0.00";
        if (IsObjectValid(in_object) &&
            (in_object.pose_reliable || in_object.label != "unknown") &&
            (in_object.dimensions.x + in_object.dimensions.y + in_object.dimensions.z) < object_max_linear_size_)
        {
            // get the score from in_object.score
            score_str = std::to_string(in_object.score);
        }else if (in_object.label == "unknown"){
            score_str = std::to_string(in_object.score);
        }else if (in_object.label != "unknown"){
            score_str = std::to_string(in_object.score);
        }
        ...
        if (std::stod(box.text) == 0.0){
            
            ROS_INFO("score_str:%s", score_str.c_str());
            ROS_INFO("box.text:%s", box.text.c_str());
            ROS_ERROR("score is 0.0");
        }

    }

    if (!object_markers.markers.empty()) {
        publisher.publish(object_markers);
    }
}

// zzl modified
static void publishObjectsMarkersFusionBoxScore(
    const ros::Publisher &publisher,
    const std_msgs::Header &header,
    const std_msgs::ColorRGBA &color,
    const std::vector<ObjectPtr> &objects_array,
    ...
    for (size_t obj = 0u; obj < objects_array.size(); ++obj) {
        ...
        if (verify){
            ...
            geometry_msgs::Point start_point, end_point;
            Eigen::Vector3d dir_v(cos(yaw_rad), sin(yaw_rad), 0);
            Eigen::Vector3d start, end;

            // check if dir_v == dir 
            if (dir_v.isApprox(dir)){
                ROS_INFO("dir_v and dir is the same!"); // didn't print it out
            }

            // print out yaw rad using the folloinwg 
            // tested: yaw_rad == yaw
            ROS_INFO("Yaw rad: %f", yaw_rad); 
            ROS_INFO("Yaw from dir: %f",yaw);

            if (dir_v.dot(velocity) < 0) {
                start = center - dir_v * (length / 2);
            } else {
                start = center + dir_v * (length / 2);
            }
            end = start + velocity;
            ...
            end_point.y = end(1);
            end_point.z = end(2);
            box.points.push_back(start_point);
            box.points.push_back(end_point);
        }
        */

        // ------------------------------- object velocity----------------------------------------
        double vel_scalar = sqrt(pow(velocity[0], 2.0) + pow(velocity[1], 2.0));
        std::stringstream vel_str;
        vel_str << std::fixed << std::setprecision(2)
                << std::setfill('0') << vel_scalar;        

        // ------------------------------- object tracker id----------------------------------------
        double obj_id=objects_array[obj]->tracker_id;
        ...
        // put all of them into the box.text
        // format: text: "6.14 m/s , <ID= 4>, <ds= 0.958259>, <ss= 0.0,0.0,0.0>, <kds= 0.0>"
        std::string ss_kds_str = ", <ss= 0.0,0.0,0.0>, <kds= 0.0>";
        box.text = vel_str.str() + " m/s" + ", <ID= "+obj_id_str.str()+">" + ", <ds= " + score_str + ">"+ss_kds_str;
...
static void publishObjectsMarkersFusionBoxScoreLabel(
    const ros::Publisher &publisher,
    const std_msgs::Header &header,
    const std_msgs::ColorRGBA &color,
    const std::vector<ObjectPtr> &objects_array,
    ...
    for (size_t obj = 0u; obj < objects_array.size(); ++obj) {
        ...
        box.header = dir_arrow.header = header;
        // box.ns = dir_arrow.ns = "objects/" + header.frame_id;
        box.ns = dir_arrow.ns = header.frame_id;
        ...
        std::string score_str="0.00";
        std::string detection_label = "unknown_0";

        // get the detection score and label here 
        if (IsObjectValid(in_object) &&
            (in_object.pose_reliable || in_object.label != "unknown") &&
            (in_object.dimensions.x + in_object.dimensions.y + in_object.dimensions.z) < object_max_linear_size_)
        {
            // get the score from in_object.score
            score_str = std::to_string(in_object.score);
            detection_label = in_object.label;
        }else if (in_object.label == "unknown"){
            score_str = std::to_string(in_object.score);
            detection_label = in_object.label;
        }else if (in_object.label != "unknown"){
            ...
            // score_str = std::to_string(in_object.score);
            score_str = "0.0";
            detection_label = "unknown_00";
        }
        
        box.ns = box.ns + "/" + detection_label;
        // -------------------------------dectection score----------------------------------------
...
// new header topic: zzl is done!
static void publishObjectsMarkersBoxScoreLabel_header(
    const ros::Publisher &publisher,
    const std_msgs::Header &header,
    const std_msgs::ColorRGBA &color,
    const std::vector<ObjectPtr> &objects_array,
    ...
    /*
    // clear all markers before
    visualization_header_msgs::MarkerArrayHeader empty_markers_header;
    // visualization_msgs::MarkerArray empty_markers;
    visualization_msgs::Marker clear_marker;

    empty_markers_header.header = header;
    clear_marker.header = header;
    clear_marker.ns = "objects";
    clear_marker.id = 0;
    clear_marker.action = clear_marker.DELETEALL;
    clear_marker.lifetime = ros::Duration();
    empty_markers_header.markers.push_back(clear_marker);
    publisher.publish(empty_markers_header);
    */
    ...
    // visualization_msgs::MarkerArray object_markers;
    visualization_header_msgs::MarkerArrayHeader object_markers_header;
    object_markers_header.header = header;
    for (size_t obj = 0u; obj < objects_array.size(); ++obj) {
        ...
        object_markers_header.markers.push_back(box);

    }

    if (!object_markers_header.markers.empty()) {
        publisher.publish(object_markers_header);
    }
}

// zzl_modified = done! 
static void publishObjectsMarkersFusionBoxScore_header(
    const ros::Publisher &publisher,
    const std_msgs::Header &header,
    const std_msgs::ColorRGBA &color,
    const std::vector<ObjectPtr> &objects_array,
...
// zzl working on it
static void publishObjectsMarkersDetBoxScoreLabel(
    const ros::Publisher &publisher,
    const std_msgs::Header &header,
    const std_msgs::ColorRGBA &color,
    const std::vector<ObjectPtr> &objects_array,
    ...
    if (!objects_array.empty()) {
        ...
        for (size_t obj = 0u; obj < objects_array.size(); ++obj) {
            ...
            if (vel_scalar > common::EPSILON) {
                ...
                vel_str << std::fixed << std::setprecision(2)
                        << std::setfill('0') << vel_scalar;
                vel_text.text = vel_str.str() + " m/s"+" , <ID= "+obj_id_str.str()+">";
            }else{ // modified by zzl 
                std::stringstream vel_str;
                std::stringstream obj_id_str;
                obj_id_str<<obj_id;
...
static void publishTrackingObjects_local_frame(
    const ros::Publisher &publisher,
    const std_msgs::Header &header,
    const std::vector<ObjectPtr> &tracking_objects)
{
    ...
    if (!tracking_objects.empty()) {
        autosense_msgs::TrackingObjectArray_local_frame msg_tracking_objects;
        msg_tracking_objects.header = header;

        ROS_INFO_STREAM("XX  Tracked objects' frame id is " << msg_tracking_objects.header.frame_id
                                      << " xx.");
        ...
        for (size_t i = 0u; i < tracking_objects.size(); ++i) {
            ...
            msg_tracking_objects.headings.push_back(heading);
            // size
