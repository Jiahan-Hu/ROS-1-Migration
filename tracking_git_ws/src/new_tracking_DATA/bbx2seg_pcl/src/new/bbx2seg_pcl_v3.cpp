...
void convert_marker2seg_pcl(const visualization_msgs::Marker& marker, sensor_msgs::PointCloud2& cloud)
{
    ...
    */
    ...
    distance_duration = 0.1;
    // diagnose 
    std::vector<Eigen::Vector3d> segment_AG = create_3dpoints(bottom_quad[0], bottom_quad[6], distance_duration);
    cloud_point_raw.insert(cloud_point_raw.end(), segment_AG.begin(), segment_AG.end());   

    std::vector<Eigen::Vector3d> segment_CE = create_3dpoints(bottom_quad[2], bottom_quad[4], distance_duration);
    cloud_point_raw.insert(cloud_point_raw.end(), segment_CE.begin(), segment_CE.end());       
