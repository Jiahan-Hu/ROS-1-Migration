...
void boundingBoxCallback(const visualization_msgs::MarkerArray::ConstPtr& msg)
{
    ...
    for (const auto& marker : msg->markers)
    {
        ...
        if (marker.type == visualization_msgs::Marker::CUBE && marker.action == visualization_msgs::Marker::ADD)
        {
            ...
            // Compute the yaw angle of the marker
            double yaw = 2.0 * atan2(marker.pose.orientation.z, marker.pose.orientation.w);

            // Generate the 32-line point cloud for each vertical face
            for (int i = 0; i < 8; ++i)
            {
                for (int j = 0; j < 4; ++j)
                {
                    ...
                    point.z = position.z + scale.z * (i < 4 ? -0.5 : 0.5);
                    // Apply yaw angle to the point
                    double cosYaw = cos(yaw);
                    double sinYaw = sin(yaw);
                    double rotatedX = point.x * cosYaw - point.y * sinYaw;
                    double rotatedY = point.x * sinYaw + point.y * cosYaw;
                    point.x = rotatedX;
                    point.y = rotatedY;
