...
class FusionNode:
    ...
    def marker_report(self,location):
        if self.fusion_markers.markers and self.fusion_markers.markers[0].ns == '':
            rospy.logwarn("marker is problematic!")   
            # rospy.logwarn("location:")
            print("location:",location)

    def double_repub_tag1_debug(self):
        print("============== debug begin short==================")
    ...
    def keep_ego_data(self,fusion_markers,together_pairs,header):
        ...
        fusion_markers 
        """
        # modified_markers = fusion_markers.markers.copy()
        for pair in together_pairs:
            if len(pair) > 1:
                last_item = pair[-1]
                for item in pair:
                    if item != last_item:
                        fusion_markers.markers[item] = Marker() # modify this line of code, remove fusion_markers.markers[item] from fusion_markers
                        fusion_markers.markers[item].header = header
                        # keep the last of item of pair and remove the other from fusion_markers
                        # print(fusion_markers.markers[item])
                        # del fusion_markers.markers[item]
                        # del modified_markers[item]
        # fusion_markers.markers = modified_markers                    
