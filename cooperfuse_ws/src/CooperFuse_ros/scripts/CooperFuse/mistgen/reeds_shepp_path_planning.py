...
def main():
    ...
    print("Reeds Shepp path planning time:", run_time, "seconds")
    print("xs:",xs)
    print("ys:",ys)
...
def remove_duplicates(xs, ys):
    ...
    for x, y in zip(xs, ys):
        rounded_x = round(x, 3)
        rounded_y = round(y, 3)
        if (rounded_x, rounded_y) not in unique_points:
            unique_points.add((rounded_x, rounded_y))
            u_xs.append(x)
            u_ys.append(y)
    return u_xs, u_ys

def remove_same_neighbor(xs,ys):
    filtered_xs = [xs[0]]
    filtered_ys = [ys[0]]
...
def vis_wayp(xs,ys,new_fig=True):
    """
    Visualize waypoints
    :param xs: waypoints x coordinate
    :param ys: waypoints y coordinate
    :return: No
    """
    if show_animation:  # pragma: no cover
        if new_fig:
...
if __name__ == '__main__':
    ...
    print("xs:",xs)
    print("ys:",ys)
    uxs, uys = remove_duplicates(xs, ys)
    print("uxs:",uxs)
    print("uys:",uys)
    vis_wayp(xs,ys)
