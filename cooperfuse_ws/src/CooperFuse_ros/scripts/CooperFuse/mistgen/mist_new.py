...
from CooperFuse.mistgen.utils.T_functions import arrangeT
from CooperFuse.mistgen.utils.T_functions import init_T
from CooperFuse.mistgen.utils.cvxopt_qp import quadprog
from CooperFuse.mistgen.utils.other_utils import computeQ
from CooperFuse.mistgen.utils.other_utils import calc_tvec
from CooperFuse.mistgen.utils.other_utils import re_shape_vaj
from CooperFuse.mistgen.utils.other_utils import poly_val
...
class mist_generator():
    ...
    def minimum_snap_2d(self,waypts,ts,v0,a0,ve,ae):
        ...
        """
        polys_x,Q_x,px = self.minimum_snap_single(waypts[0],ts,v0[0],a0[0],ve[0],ae[0])
        polys_y,Q_y,py = self.minimum_snap_single(waypts[1],ts,v0[1],a0[1],ve[1],ae[1])
        Ex = np.dot(np.transpose(px), np.dot(Q_x, px))
        Ey = np.dot(np.transpose(py), np.dot(Q_y, py))
        return polys_x,polys_y,Ex,Ey
    ...
    def mist_2d_gen(self,waypts,v0,a0,ve,ae,T=None,ave_speed=None):
        ...
        self.polys_x,self.polys_y,Ex,Ey = self.minimum_snap_2d(waypts,self.ts,v0,a0,ve,ae)
