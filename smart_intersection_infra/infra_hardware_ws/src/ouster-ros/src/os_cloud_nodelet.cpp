   ...
   private:
    ...
    void convert_scan_to_pointcloud_publish(std::chrono::nanoseconds scan_ts,
        ...
        for (int i = 0; i < n_returns; ++i) {
            ...
            // TODO: substract the nsec 0.4 s 
            // pc_ptr->header.stamp = msg_ts;
            ros::Time modifiable_ts = msg_ts;

            if (modifiable_ts.nsec < 400000000){
                modifiable_ts.sec = modifiable_ts.sec - 1;
                modifiable_ts.nsec = modifiable_ts.nsec + 600000000;
            }else{
                modifiable_ts.nsec = modifiable_ts.nsec - 400000000;
            }

            pc_ptr->header.stamp.sec = modifiable_ts.sec;
            pc_ptr->header.stamp.nsec = modifiable_ts.nsec;
